...
def cut_cook_config(expanded_app_path,cutpath_app_path,appname,cutoutpath_app_path):
    ...
    for dir in ['default','local']:
        ...
        with open(os.path.join(cutpath_app_path,dir,"transforms.conf"), 'w') as configfile:
...
def main(tmpdirname: str,url: str):
    ...
    local_filename=getUrl(tmpdirname,url)    
    ...
    if pargs.cut:
        ...
        if os.path.exists(os.path.join(expanded_app_path,"app.manifest")):
            shutil.copy(os.path.join(expanded_app_path,"app.manifest"),cutpath_app_path)
        if os.path.exists(os.path.join(expanded_app_path,"default", "app.conf")):
            shutil.copy(os.path.join(expanded_app_path,"default", "app.conf"),os.path.join(cutpath_app_path,"default"))

        
        
        cutoutpath_app_path = os.path.join(outputdir,"cut")
        cut_cook_config(expanded_app_path,cutpath_app_path,appname,cutoutpath_app_path)
    elif pargs.base:
    ...
    if pargs.sh:
        ...
        if "base" in pargs.sh:            
            ...
            shutil.copy(local_filename,os.path.join(os.path.join(droot,"base"),appname + ".tgz"))
        else:
            for d in pargs.sh:
                os.mkdir(os.path.join(droot,d))
                shutil.copy(local_filename,os.path.join(os.path.join(droot,d),appname + ".tgz"))
